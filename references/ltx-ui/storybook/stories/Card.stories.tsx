...
const Card: React.FC<{ variant?: string; size?: string }> = (args) => {
    const style = {
    ...
    return React.createElement('div', { style, 'data-variant': args.variant, 'data-size': args.size },
      'Card content'
    );
  };
...
const meta: Meta<typeof Card> = {
  ...
  tags: ['autodocs'],
  argTypes: {
    variant: { control: 'select', options: ['outline'] },
    size:    { control: 'select', options: ['md'] },
  },
  parameters: {
    docs: {
      description: {
        component: 'card — 55 instances detected across the page.',
      },
    },
  },
};
...
export const Sizes: Story = {
  render: () => React.createElement('div', { style: { display: 'flex', gap: 12, alignItems: 'center' } },
    React.createElement(Card, { key: 'md', variant: 'outline', size: 'md' })
  ),
};
